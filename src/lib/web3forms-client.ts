...
export async function submitWeb3Forms(accessKey:string,subject:string,data:Record<string,string>){
  if(!accessKey)throw new Error("This form is temporarily unavailable. Please email hello@digiudyam.com.");
  ...
  if(!response.ok||!result?.success)throw new Error(result?.message||result?.body?.message||"We could not send your request. Please email hello@digiudyam.com.");
}
