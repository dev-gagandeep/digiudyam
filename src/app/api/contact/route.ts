...
export async function POST(request:NextRequest){
  ...
  try{await sendWeb3Forms("contact",`Contact enquiry: ${business}`,{name,business_name:business,phone,email,website:website||"Not provided",business_type:businessType,help_required:help,message,replyto:email,page_url:"https://digiudyam.in/contact"});return NextResponse.json({ok:true})}catch(error){console.error("Contact form delivery failed",{code:error instanceof Error?error.message:"UNKNOWN"});return NextResponse.json({error:"We could not send your enquiry. Please email hello@digiudyam.com."},{status:503})}
}
